...
let handler = async (m, { conn, text }) => {
   ...
   if (text.length > 60) return m.reply('Maksimal 60 Teks!')
...
try {
    let as = `${webapi}api/canvas/quotes?nama=${m.name}&username=@${name}&kata=${text}&ppurl=${ppurl}&q=Trusted%20IchanDev&apikey=${apichan}`
